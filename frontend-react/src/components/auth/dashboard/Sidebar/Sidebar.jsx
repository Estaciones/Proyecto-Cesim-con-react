...
export default function DashboardSidebar() {
  const { activeSection, setActiveSection, profile, user, openModal } = useContext(DashboardContext);
  ...
  const allowedItems = baseMenuItems.filter(item => item.allowed.includes(userType));

  // Construir el orden final respetando la lista de alloweds
  let menuItems = [];
  if (userType === 'medico' || (typeof userType === 'string' && userType.includes('gestor'))) {
  ...
  const filteredQuickActions = quickActions.filter(action => action.allowed.includes(userType));
