...
export function DashboardProvider({ children }) {
  ...
  const [currentRegistroPacienteId, setCurrentRegistroPacienteId] = useState(null);
    ...
    try {
      ...
      if (user?.tipo === 'paciente' || profile?.tipo_usuario === 'paciente') {
        ...
        if (selectedPatient.ci) {
          url = apiUrl(`plan_tratamiento?ci=${encodeURIComponent(selectedPatient.ci)}`);
        } else if (selectedPatient.id_paciente) {
          url = apiUrl(`plan_tratamiento?id_paciente=${selectedPatient.id_paciente}`);
        }
      } else if (profile?.tipo_usuario === 'medico' || profile?.tipo_usuario?.includes('gestor')) {
  ...
  const createPlan = useCallback(
    async ({ titulo, descripcion, fecha_inicio, id_paciente, prescripciones }) => {
      try {
        const payload = {
  ...
  const updatePlan = useCallback(
    async ({ planId, titulo, descripcion, fecha_inicio, estado, resumen_egreso }) => {
      try {
...
const createPatient = useCallback(
  ...
  const canViewPatients = useCallback(() => isMedico() || isGestor(), [isMedico, isGestor]);
  ...
  }, [profile, activeSection, selectedPatient, loadPatients, loadPlanes, loadHistoria]);
